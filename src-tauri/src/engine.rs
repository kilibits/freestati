...
impl Engine {
    ...
    pub fn get_page(&self, offset: usize, limit: usize, query: Option<String>, filter_col: Option<String>) -> EResult<PageResult> {
        let Some(df) = &self.df else {
        ...
        if let Some(q) = query {
            ...
            if let Some(col_name) = filter_col {
                mask = Some(col(col_name.as_str())
